...
#SBATCH --output "/data/keeling/a/snesbitt/wrf_realtime/scripts/slurm-%A_%a.out"
...
jid=`sbatch plotting_test.sh ${run}| awk '{print $NF }'`

sbatch --dependency=afterany:$jid webwork.sh ${run}

wps_dir=/data/snesbitt/f/snesbitt/wrf/domains/sa_3km/${run}
